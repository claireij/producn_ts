  ...
  try {
    ...
    if (req.method === "POST") {
      ...
      const contextText = validContext
        ...
        const neededResult = "Based on the user input, provide a clear diagnosis of the producer's problem and concrete, actionable next steps tailored to music production."

        const generatedText = await AnthropicService.createNewResult({contextText: `Here is the user's diagnostic Q&A session:\n\n${contextText}`, neededResult: neededResult })
